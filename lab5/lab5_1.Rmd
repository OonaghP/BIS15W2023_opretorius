...
If all of the characters should be factors, 
NOTE: shift+command/control +M will give you pipes
```{r}
mammals %>% mutate_if(is.character, as.factor)
glimpse(mammals) #shows that allthe characters were converted to factors.
```
...
can't use the & becaus then it will want to satisfy both conditions, which is impossible. You can use "|"
```{r}
fish %>% 
  filter(lakeid == c("AL", "AR")) %>% 
  filter(between(radii_length_mm, 2, 4)) #check if includes the extremes
#or filter(radii_length_mm >= 2 & radii_length_mm <= 4)
```
...
mammals %>% 
  ...
  arrange(newborn)
```
ifelse: makes a new column called newborn_new. If in the column newborn there is a -999, put an NA, else, just transpose the newborn column's value.
```{r}
mammals %>% 
  select(genus, species, newborn) %>%
