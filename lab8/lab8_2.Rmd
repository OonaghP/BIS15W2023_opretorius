...
## Practice
1. Are these data tidy? 
No, because the columns contain information

Please use `pivot_longer()` to tidy the data.
```{r}
expenditures %>% 
...
## Practice
1. Load the `gene_exp.csv` data as a new object `gene_exp`. Are these data tidy? 
No, there are two different types of variables nested within "type".

Use `pivot_wider()` to tidy the data.
```{r}
...
```{r}
beach_bugs <- readr::read_csv("data/beachbugs_long.csv")
beach_bugs
```

2. Use `pivot_wider` to transform the data into wide format.
```{r}
beach_bugs2 <- beach_bugs %>% 
  pivot_wider(
    names_from = "site", 
