...
```{r}
here() #always put you in the main directory!???
```
You just need to tell it where to find your data
```{r}
heartrate <- read_csv(here("data2", "heartrate.csv")) #"lab 8"
```
...
(1) each variable has its own column?  
 No, because the different drugs are also data and they should not be column headings
...
**NB to format data for R to use for plots**
```{r}
heartrate %>% 
...
Data are included in the column headings and should be observations.
...
plant_data %>% 
  pivot_longer(-c(genotype, water_sched_prog, greenhouse), #specific columns
               names_to = "Day",
               values_to = "Variable 1",
               values_drop_na = TRUE
               )
```
## Easy to record data in wide format, but we must change it to long format before we work with it.
