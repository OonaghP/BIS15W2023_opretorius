...
a) Here the classes of the data are labeled in a data frame.
b) opens a little spread sheet
c) can sort
d) can ask questions
```{r}
...
str(hbirds)
```
Notice that all the names in the df except for the variable names are lower case. Better to make all lower-case so you don't get stuck on code later on. Also no spaces!
...
```
This will now appear in your folder.
...
```{r}
spring_mean <- mean(hsprings$temp_C)
spring_mean
```
