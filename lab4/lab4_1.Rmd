...
### **We use " " here to tell R that the column name is NOT an object.**
...
## **NB** helps you to find things that start with something and end with something.
...
```
No, not technically, but the code -999 is used instead of NAs or blanks, so this must be addressed.
...
```{r}
#select_all(mammals, ~str_replace(., " ", "_")) #the last character here is what you are replacing everything with.
```
