...
NOTE: An object will appear in the Environment. It can be many things e.g. vectors, values, etc. 
...
## `filter()`
select() looks in columns (works vertically), whereas filter() looks inside the rows.
...
```{r}
mammals2 <- data.frame(select(mammals, "family", "genus", "species", "mass", "gestation"))
mammals2
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
filter(mammals2, family == "Bovidae" & mass > 450000)
```
...
```{r}
mammals3 <- data.frame(select(mammals, "genus", "species", "mass", "gestation", "newborn"))
mammals3
```

```{r}
mammals3 %>% 
  filter(genus=="Erythrocebus" | genus=="Lophocebus" | genus=="Macaca")
```


## **QUESTION**
R won't recognise "Erythrocebus" for some reason. I checked the class but it still didn't recognise it when changed to character or factor.
```{r}
mammals4 <- as.data.frame(filter(mammals3, genus == c("Lophocebus","Erythrocebus","Macaca")))
mammals4
```
In the end I had to filter it seperately and then bind it to the dataframe.
```{r}
Erythrocebus <- data.frame(filter(mammals3, genus == "Erythrocebus"))
```

```{r}
mammals5 <- rbind(mammals4, Erythrocebus)
mammals5
```
We can see from the dataframe that Macaca silenus has the smallest newborn mass, but we can also use which.min(), which tells us it is the 4th observation.
```{r}
which.min(mammals5$newborn)
```
