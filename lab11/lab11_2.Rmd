...
## RColorBrewer
The default colors used by ggplot are often uninspiring. They don't make plots pop out in presentations or publications, and you may want to use a customized palette to make things visually consistent.  Developed for mapping.
...
+`scale_colour_brewer()` is for points like in a scatter plot
+`scale_fill_brewer()` is for fills for blocks/shapes
...
```{r}
my_palette <- paletteer_d("ggsci::lanonc_lancet")
```
...
2. Try the same plot as #1 above, but include life_expectancy(a)~habitat(b).
```{r}
ecosphere %>% 
  ggplot(aes(x=log10_mass))+ 
  geom_density(alpha=0.4)+ 
