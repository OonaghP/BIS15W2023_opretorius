...
```{r}
homerange %>% 
  count(class, taxon) #want to show this in the plot
```
...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon)) + #doesn't include colour
  geom_boxplot()
```
...
```{r}
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, fill = taxon)) + # don't need to group again
  geom_boxplot()
```
...
```{r}
deserts2 <- deserts %>% mutate(year=as_factor(year)) #if you don't do this, then R will only show you years in 5 year gaps (as the data was entered in 5 year intervals)
```
...
deserts2 %>% 
  filter(species_id=="DM" | species_id=="DS") %>% 
  group_by(year, species_id) %>% 
  summarise(n=n(), .groups='keep') %>% #(.groups='keep') allows you to keep the grouping per year and then won't give you an error. You could be grouping by species or year
  ggplot(aes(x=year, y=n, group=species_id, color=species_id))+
  geom_line()+ #up to here shows trends
  geom_point(shape=5)+
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, color = "black", fill = "firebrick3", bins=40)+ #if you don't specify, R will guess what binwidth to use
  labs(title = "Distribution of Body Mass")
```

Let's play with the colors. This shows all 657 of R's built-in colors. Notice that `color` (outline of bar) and `fill`(inside of bar) do different things!
```{r}
grDevices::colors()
```
...
homerange %>% 
  ...
  geom_histogram(aes(y = after_stat(density)), fill = "salmon", alpha = 0.4, color = "black", bins = 40)+
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = after_stat(density)), fill = "salmon", alpha = 0.5, color = "black", bins = 40)+
...
```{r}
homerange <- homerange %>% 
             mutate(hra_category = case_when(log10.hra <= 3.65 ~ "Q1",
                                              log10.hra> 3.65 & log10.hra <= 4.59 ~ "Q2",
                                              log10.hra> 4.59 & log10.hra <= 6.02 ~ "Q3",
                                              log10.hra > 6.02 ~ "Q4"))
```
...
homerange %>% 
  ...
  geom_bar(position="dodge")+
  labs(title = "Observations by Class and Home Range Area in Homerange Data",
       x = "HRA Category",
       fill = "Class")
```
