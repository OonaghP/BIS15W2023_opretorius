...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) #lapply is part of base R. It will iterate over all "files" and find all those that are csvs
```

We can view elements in our list with double brackets. Let's view the data for Butte county.
```{r}
spider_list[[3]] # indexes number 3 in the list
```
...
## Practice
You can ask for specific counties in the list now
```{r}
...
`inner_join`  
Where did customers 3 and 4 go? Only joined and kept those customer IDs that appeared in both tables, and drops whatever does not match exactly in the joining criteria. Joins *between* the tables.
```{r}
...
`left_join`
Where did customer 4 go? Notice the missing data for customer 3. Only kept customers in the first table
```{r}
...
`right_join`
Where did customer 3 go? Notice the missing data for customer 4. Only kept customers listed in the second table
```{r}
...
`full_join` 
Here is keeps all customers present in either table A or B. Best option. Rather deal with the missing data than have data removed.
```{r}
...
`anti_join`
Provides (lists) the rows in the first table for which there are not matching values in the second table. Useful for seeing what you might miss if you use joins that are not full_join.
```{r}
...
```{r}
spiders_with_locs <- 
  left_join(spiders_all, spiders_locs, by = "Accession") #must use a unique identifier
...
day
```
What is the structure of this day?
```{r}
str(day)
```
...
## Export for part 2  
*Saving our new joined datasets*
We need to save our final data to a `.csv` so we can use it in part 2. 
```{r}
write.csv(spiders_with_locs, file = "spiders_with_locs.csv", row.names = FALSE) #don't include row names or it will give each row a number.
```
