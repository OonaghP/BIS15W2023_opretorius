...
```{r}
glimpse(homerange) #It looks tidy, but there are many NAs.
...
ggplot(homerange, mapping = aes(x=log10.mass, y=log10.hra)) + 
  geom_point() +
  geom_smooth(method = lm, se = TRUE) #tells it you want a straight line and std error. If you don't specify, it can curve.
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?
```{r}
ggplot(homerange, mapping = aes(x=log10.preymass, y=log10.hra)) +
  geom_point() +
  geom_smooth(method = lm, se = TRUE)
```

There is a positive linear relationship.
ggplot ignores the NAs.
...
### Bar Plot: `geom_col()`
Unlike `geom_bar()`, `geom_col()` allows us to specify an x-axis (always categorical!) and a y-axis.
```{r}
homerange %>% 
  filter(family=="salmonidae") %>%
  select(common.name, log10.mass) %>% 
  ggplot(aes(x=common.name, y=log10.mass))+ #once you specify the axes, you have to use geom_col()!
  geom_col()
```
...
homerange %>% 
  filter(family=="salmonidae") %>% 
  ggplot(aes(x=common.name, y=log10.mass))+ #can flip on its side if you specify the x first
