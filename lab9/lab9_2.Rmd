...
Make this output more visual by making a plot... **??CHECK**
```{r}
homerange %>% 
  group_by(class) %>% 
  summarize(min_log10.mass=min(log10.mass),
  ...
  ggplot(aes(x = class))+
  geom_boxplot(
    aes(ymin = min_log10.mass, middle = median_log10.mass, ymax = max_log10.mass)
  )
```
...
homerange %>% 
  ...
  coord_flip()+
  geom_boxplot(
    aes(xmin = min_log10.mass, middle = median_log10.mass, xmax = max_log10.mass)
  )
```
