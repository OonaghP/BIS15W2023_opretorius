...
msleep24 <- msleep %>% 
  ...
  filter(!is.na(vore)) %>% 
  arrange(desc(sleep_total_24 < 0.3))
msleep24
```
...
msleep %>% 
    ...
    summarize(mean_prim_wt = mean(bodywt), 
              min_prim_wt = min(bodywt),
              max_prim_wt = max(bodywt),
              total = n())
```
...
```{r}
msleep %>%
  group_by(order) %>% #we are grouping by order, a categorical variable
...
msleep %>% 
  ...
  filter(order == "Carnivora"| order == "Cetacea")
```
In Cetacea (whales) there are no values for brainwt, whereas in Carnivora there are some which which to perform the calculation.
