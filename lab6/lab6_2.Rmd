...
```
Gentoo and Adelie penguins are missing data for body mass.
...
You can use tabyl too. Organisation is just a bit different.
...
## **NB: group_by(), summarize() and count() will be on the midterm!**
...
penguins %>% 
  filter(is.na(sex)) %>% 
  tabyl(island) #or use count(island)
```
