...
```{r}
p+geom_point(aes(shape=thermoregulation, color=thermoregulation), size=1.75) #change the shape of the point by thermoregulation variable
```
...
```{r}
homerange %>% count(taxon, sort = TRUE)
```
...
homerange %>% 
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild") #can be a bit hard to read
```

We can also have counts of each trophic guild within taxonomic group shown side-by-side by specifying `position="dodge"`.
```{r}
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) + geom_bar(position = "dodge") + #doge puts them side by side
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild")
